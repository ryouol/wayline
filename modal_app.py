"""Research-only Modal runner using durable object references.

This module is intentionally not wired to billing or enabled by the workspace.
It uploads this checkout's source rather than cloning a moving upstream branch,
uses per-job temporary directories, validates archive members, verifies model
digests, and writes GLB output to a Volume instead of returning a large RPC
payload.
