...
| Sticky mobile CTA | ✅ The public mobile header keeps Get started visible while scrolling; the mobile workspace source column keeps New scene sticky until a viewer is visible. A loaded scene lets that column scroll away so walking controls and the canvas fit together; New scene remains above the viewer. Browser inspection measured the public header at top 0 after scrolling. These existing navigation areas provide the sticky actions; the unused floating CTA is not the implementation. |
...
| ✅ Fitts | Large primary actions, 44px control targets, nearby viewport tools, sticky mobile Get started, New scene above the studio, and walking controls placed directly on the viewport. |
...
The [actual account-scene check](GOOGLE_CAPTURE_QA.md) now verifies saved-output viewer/replay/walk and local sharing, with an explicit visual-quality failure for the assembled geometry. It does not substitute for owned-phone or second-account acceptance. The active-thumbnail and plain-language shared-rights follow-up fixes two observed UI issues; the [mobile walk follow-up](MOBILE_WALK_QA.md) places movement directly on the viewer and verifies simultaneous scene/control visibility in local narrow layouts. Physical-device acceptance remains open. [Source notice QA](SOURCE_NOTICE_QA.md) records the identified license texts now packaged in the distributions without claiming complete source or hosted-use clearance.
