...
- A Dockerfile and single-instance Starter/5 GB Render Blueprint validate. The Wayline project, Production environment, Starter service and disk exist. The user accepted $20/month as a target with flexibility. Application delivery/GPU allowances reduce exposure without guaranteeing a bill total. The root-owned disk permission issue is fixed with `/data/wayline`. [Render QA](RENDER_QA.md) records the generated-input original-model run, visitor isolation, shares and redeploy persistence; [the latest deployment receipt](DIALOG_LOADING_QA.md) tracks runtime `abd3f43` with modal-visible pending feedback and analytics disabled, [onboarding QA](ONBOARDING_QA.md) records the bounded pilot, and [recovery QA](SCHEDULED_RECOVERY_QA.md) records the backup rollout.
...
The [dialog feedback follow-up](DIALOG_LOADING_QA.md) fixes pending feedback behind native dialogs and asynchronous modal ordering. A [new Google-account reconstruction](GOOGLE_CAPTURE_QA.md) reached READY in 77.388 seconds with 749,963 finite points and settled usage; ownership, physical-device and visual acceptance remain unconfirmed. Five of six GPU admissions are now used. [Provenance](../MODEL_PROVENANCE.md) now identifies conditional Waymo trained-model/service restrictions; free hosted testing is part of the unresolved rights scope.
