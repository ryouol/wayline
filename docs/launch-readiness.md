...
- A Dockerfile and single-instance Starter/5 GB Render Blueprint validate. The Wayline project, Production environment, Starter service and disk exist. The user accepted $20/month as a target with flexibility. Application delivery/GPU allowances reduce exposure without guaranteeing a bill total. The root-owned disk permission issue is fixed with `/data/wayline`. [Render QA](RENDER_QA.md) records the generated-input original-model run, visitor isolation, shares and redeploy persistence; [the source-notice deployment receipt](SOURCE_NOTICE_QA.md) tracks runtime `5e1d370`, including identified third-party license texts, with analytics disabled, [onboarding QA](ONBOARDING_QA.md) records the bounded pilot, and [recovery QA](SCHEDULED_RECOVERY_QA.md) records the backup rollout.
...
The subsequent recovery release `d88da15` passed 237 Python tests, ten Node suites
and exact-commit CI. Live health, all 64 static assets, the recovery source and all
12 saved artifact hashes passed; saved jobs and usage counters remained intact.
Crash tests now preserve uncertain completion acknowledgements, and remote
scheduled storage is bounded to eight known prefixes with conservative cleanup.
...
The [dialog feedback follow-up](DIALOG_LOADING_QA.md) fixes pending feedback behind native dialogs and asynchronous modal ordering. A [new Google-account reconstruction](GOOGLE_CAPTURE_QA.md) reached READY in 77.388 seconds with 749,963 finite points and settled usage; source ownership and physical device remain unconfirmed. Saved-output browser checks now verify frame selection, replay, walking/reset and local signed-out sharing, but the assembled scene fails visual-quality acceptance. The locally verified viewer follow-up keeps active replay thumbnails visible and replaces the raw shared rights label with plain language; its release receipt is separate from these saved-output checks. Five of six GPU admissions are now used, and the Google account has consumed its one successful video. [Provenance](../MODEL_PROVENANCE.md) now identifies conditional Waymo trained-model/service restrictions; free hosted testing is part of the unresolved rights scope.
...
- [ ] Validate provider limits/alerts and the remaining failure/crash cleanup boundaries. Successful-run cleanup, due-record cleanup after restart, and the live running-inference cancellation/grace-period case passed. Local SIGKILL tests now cover process death after a private artifact transaction commits, before job completion, partial-artifact privacy, natural lease expiry and exactly-once settlement. Broader provider crash boundaries remain open. The app's admission budget is not a provider invoice cap.
...
- [ ] Validate recovery at the full workspace capacity, configure external failure/staleness alerts, escrow the recovery key separately, and run an isolated replacement-Render recovery drill. A local offline drill restored 3.49 GB (99.71% of the workspace ceiling) under a 512 MiB limit with no OOM, but memory pressure was substantial. It used a disk-backed fake volume without concurrent service workers; provider/concurrent-service recovery remains unverified. See `RECOVERY_CRASH_QA.md`.
