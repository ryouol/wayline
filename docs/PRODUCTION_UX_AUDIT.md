...
- Finish the remaining deployed load/shutdown/crash-cleanup checks. Current redesign source scanning, simplify/code review, clean wheel/container, hosted CI and deployment verification passed for runtime `9516dbf`; [the release receipt](DESIGN_RELEASE_QA.md) records the actual revision and scope. Aggregate PR-size review concerns remain open; the draft PR is unmerged.
